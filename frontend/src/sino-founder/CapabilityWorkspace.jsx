...
export function CapabilityCenter({ onContinue, onNavigateBuilder, onNavigateExecution, onNavigateAsset }) {
  ...
  async function choose(item) { setSelected(item); try { setSelected(await getLifecycleAsset(item.asset_id)); } catch (reason) { setError(reason.message); } }
  return <section className="sino-lifecycle-center sino-primary-center" aria-label="AI 能力中心">
    ...
    <div className="sino-lifecycle-grid"><div className="sino-asset-list">{visible.length ? visible.map((item) => <button type="button" key={item.asset_id} className={selected?.asset_id === item.asset_id ? "is-active" : ""} onClick={() => choose(item)}><span>{objectTypeLabel(item.asset_type)}</span><strong>{businessAssetName(item)}</strong><p>{businessPurpose(item)}</p><small>{statusLabel(item.status)} · V{item.version || 1} · Used By {item.used_by_refs?.length || 0} · {date(item.updated_at)}</small></button>) : <div className="sino-business-empty"><strong>暂无{type ? objectTypeLabel(type) : "AI 能力"}</strong><p>从 Founder 与 Sino 讨论创建一个能力，批准成果包后会进入这里。</p></div>}</div>
      <aside className="sino-asset-detail">{selected ? <><span className="sino-kicker">{objectTypeLabel(selected.asset_type)}</span><h2>{businessAssetName(selected)}</h2><p>{businessPurpose(selected)}</p><dl><div><dt>状态 / 版本</dt><dd>{statusLabel(selected.status)} · V{selected.version || 1}</dd></div><div><dt>Used By</dt><dd>{selected.used_by_refs?.length ? selected.used_by_refs.join(" · ") : "尚未被应用引用"}</dd></div><div><dt>依赖</dt><dd>{selected.dependency_refs?.length ? selected.dependency_refs.join(" · ") : "暂无"}</dd></div><div><dt>来源</dt><dd>{selected.source_conversation_id || "历史来源不可用"}</dd></div><div><dt>Latest Learning</dt><dd>{selected.learning_refs?.at?.(-1)?.summary || "暂无"}</dd></div><div><dt>Asset ID</dt><dd>{selected.asset_id}</dd></div></dl><div className="sino-lifecycle-actions"><button onClick={() => onContinue?.(selected)}>继续讨论</button><button onClick={() => onNavigateBuilder?.(selected)}>进入系统构建器</button><button onClick={() => onNavigateExecution?.(selected)}>进入执行</button><button className="is-primary" onClick={() => onNavigateAsset?.(selected)}>查看资产记录</button></div></> : <div className="sino-inspector-empty"><h2>能力详情</h2><p>选择一个能力，查看用途、版本、引用、依赖和学习记录。</p></div>}</aside></div>
  </section>;
}

// Compatibility export for existing object routes.
