...
const connectionStatusLabel = (value) => ({ healthy: "连接正常", unhealthy: "连接异常", disabled: "已停用", pending_check: "待检查", not_configured: "未配置" }[value] || "尚未检查");

export function ExternalModelHealthCheck({ providerKey }) {
  const [provider, setProvider] = useState(null); const [loading, setLoading] = useState(true); const [authorization, setAuthorization] = useState(false); const [checking, setChecking] = useState(false); const [error, setError] = useState("");
  useEffect(() => {
    let active = true; setLoading(true); setError(""); setAuthorization(false);
    getModelCenter().then((center) => { if (active) setProvider((center.providers || []).find((item) => item.provider_key === providerKey) || null); }).catch((reason) => { if (active) setError(reason.message); }).finally(() => { if (active) setLoading(false); });
    return () => { active = false; };
  }, [providerKey]);
  const mayIncurCost = provider?.health_check_requires_external_call !== false && provider?.health_probe?.may_incur_cost !== false;
  async function runCheck() {
    setChecking(true); setError("");
    try { const result = await checkModelProvider(providerKey); setProvider((current) => ({ ...current, ...(result.configuration || {}), health_status: result.status || result.configuration?.health_status })); setAuthorization(false); }
    catch (reason) { setError(reason.message); }
    finally { setChecking(false); }
  }
  function requestCheck() { if (mayIncurCost) setAuthorization(true); else runCheck(); }
  return <section className="sino-model-health-check" aria-label="外部模型连接健康检查"><header><div><span className="sino-kicker">Model Connection</span><h3>外部模型连接</h3></div>{provider ? <strong data-health={provider.health_status}>{connectionStatusLabel(provider.health_status)}</strong> : null}</header>{loading ? <p role="status">正在读取当前连接状态…</p> : provider ? <><dl><div><dt>Provider</dt><dd>{provider.display_name || provider.provider_key}</dd></div><div><dt>配置状态</dt><dd>{provider.configured === false ? "未配置" : "已配置"}</dd></div><div><dt>最近检查</dt><dd>{provider.last_health_check_at ? date(provider.last_health_check_at) : "暂无记录"}</dd></div></dl>{authorization ? <div className="sino-model-health-authorization" role="alertdialog" aria-label="Founder 健康检查授权"><strong>需要 Founder 授权</strong><p>这次检查将真正调用外部模型 API，可能产生费用。授权前不会发起调用。</p><div><button type="button" className="is-primary" disabled={checking} onClick={runCheck}>{checking ? "检查中…" : "授权并检查"}</button><button type="button" disabled={checking} onClick={() => setAuthorization(false)}>暂不授权</button></div></div> : <button type="button" className="sino-inline-action" disabled={checking || provider.configured === false} onClick={requestCheck}>{checking ? "检查中…" : "检查连接状态"}</button>}</> : <p>当前没有找到已配置的模型连接。</p>}{error ? <p role="alert">{error}</p> : null}</section>;
}
...
function DraftTaskContent({ draft, implementation }) {
  const [expanded, setExpanded] = useState(true);
  const pkg = implementation?.execution_package;
  const workItems = pkg?.work_items || implementation?.work_items || [];
  const scope = pkg?.scope || implementation?.scope || [];
  const acceptance = pkg?.acceptance_criteria || implementation?.acceptance_criteria || [];
  const fallback = draft.summary || draft.current_next_step;
  return <section className="sino-draft-task-content" aria-label="任务内容">
    <header><div><span className="sino-kicker">Task Content</span><h3>任务内容</h3></div><button type="button" aria-expanded={expanded} aria-controls="draft-task-content-body" onClick={() => setExpanded((current) => !current)}>{expanded ? "收起任务内容" : "查看任务内容"}</button></header>
    {expanded ? <div id="draft-task-content-body">
      {implementation?.implementation_goal ? <p className="sino-draft-task-content__goal">{implementation.implementation_goal}</p> : null}
      {workItems.length ? <ol>{workItems.map((item, index) => <li key={item.work_item_id || index}><strong>{item.title || item.scope || `工作项 ${index + 1}`}</strong>{item.purpose ? <p>{item.purpose}</p> : null}{item.scope && item.scope !== item.title ? <small>范围：{item.scope}</small> : null}{item.validation ? <small>验证：{item.validation}</small> : null}</li>)}</ol> : <div className="sino-draft-task-content__fallback"><strong>执行依据</strong><p>{fallback || "任务将以当前已确认草案为执行依据；完整工作项可从原讨论中的实施方案查看。"}</p></div>}
      {scope.length ? <div><strong>范围</strong><StructuredValue value={scope} /></div> : null}
      {acceptance.length ? <div><strong>验收标准</strong><StructuredValue value={acceptance} /></div> : null}
    </div> : null}
  </section>;
}
...
function LifecycleCenter({ selected, onSelect }) {
  ...
  useEffect(() => {
    ...
    Promise.all(domains.map((item) => getCapabilityRepositoryAssets(item.domain_id, "", "")))
      .then((responses) => {
        ...
        responses.forEach((response, index) => (response.assets || []).forEach((item) => {
          if (CAPABILITY_TYPES.has(item.asset_type) || item.asset_type === "knowledge") uniqueAssets.set(item.asset_id, { ...item, domain_id: item.domain_id || domains[index].domain_id });
        }));
  ...
  const visibleDomains = useMemo(() => domains.filter((item) => !normalizedQuery || normalizeSearch(`${item.name || ""} ${item.domain_id || ""}`).includes(normalizedQuery)), [domains, normalizedQuery]);
  const globalMatches = useMemo(() => globalAssets.filter((item) => {
    ...
    return normalizedQuery && normalizeSearch(`${businessAssetName(item)} ${businessPurpose(item)} ${item.domain_id || ""} ${assetDomain?.name || ""}`).includes(normalizedQuery);
  }), [globalAssets, domains, normalizedQuery]);
  const visible = useMemo(() => assets.filter((item) => !normalizedQuery || normalizeSearch(`${businessAssetName(item)} ${item.domain_id || ""} ${selectedDomain?.name || ""}`).includes(normalizedQuery)), [assets, normalizedQuery, selectedDomain?.name]);
...
export function CapabilityCenter({ selected, onSelect, section = "drafts", onSectionChange, selectedDraft, onSelectDraft }) {
  return <section className="sino-lifecycle-center sino-primary-center" aria-label="能力仓库"><nav className="sino-repository-tabs" aria-label="能力仓库区域"><button className={section === "drafts" ? "is-active" : ""} onClick={() => onSectionChange?.("drafts")}>草案中心</button><button className={section === "lifecycle" ? "is-active" : ""} onClick={() => onSectionChange?.("lifecycle")}>能力周期</button></nav>{section === "drafts" ? <DraftCenter selected={selectedDraft} onSelect={onSelectDraft} /> : <LifecycleCenter selected={selected} onSelect={onSelect} />}</section>;
}

export function DraftContext({ selected, busy, onApproveImplementation, onViewImplementationPlan, onReviewFounderGate, onReturnConversation }) {
