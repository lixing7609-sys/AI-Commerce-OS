...
import { approveCapabilityReady, completeCapabilityDevelopment, getCapabilityDomains, getCapabilityRepositoryAssets, getLifecycleAsset, runCapabilityTest, startCapabilityDevelopment } from "../services/founderAiApi.js";
...
export function CapabilityContext({ selected, onContinue, onChanged }) {
  if (!selected) return <FounderWorkspaceInspector title="能力详情" description="选择一个能力查看上下文。" />;
  async function action(fn) { onChanged?.(await fn(selected.asset_id)); }
  return <FounderWorkspaceInspector><section className="sino-asset-detail"><span className="sino-kicker">Capability Repository</span><h2>{businessAssetName(selected)}</h2><p>{businessPurpose(selected)}</p><dl><div><dt>Domain</dt><dd>{selected.domain_id}</dd></div><div><dt>Type</dt><dd>{objectTypeLabel(selected.asset_type)}</dd></div><div><dt>Status / Version</dt><dd>{statusLabel(selected.status)} · V{selected.version || 1}</dd></div><div><dt>Source Conversation</dt><dd>{selected.source_conversation_id || "历史来源不可用"}</dd></div><div><dt>Source Package</dt><dd>{selected.source_package_id || "暂无"}</dd></div><div><dt>Test Result</dt><dd>{selected.test_run_refs?.at?.(-1)?.status || "尚未测试"}</dd></div><div><dt>Dependencies</dt><dd>{referenceSummary(selected.dependency_refs)}</dd></div><div><dt>Used By / Reuse</dt><dd>{selected.used_by_refs?.length || 0} / {selected.reference_count || 0}</dd></div><div><dt>Asset ID</dt><dd>{selected.asset_id}</dd></div></dl><div className="sino-lifecycle-actions">{selected.status === "candidate" ? <button className="is-primary" onClick={() => action(startCapabilityDevelopment)}>开发</button> : null}{selected.status === "developing" ? <button className="is-primary" onClick={() => action(completeCapabilityDevelopment)}>完成开发并进入测试</button> : null}{selected.status === "testing" ? <><button onClick={async () => { await runCapabilityTest(selected.asset_id); onChanged?.(await getLifecycleAsset(selected.asset_id)); }}>运行真实测试</button>{selected.test_run_refs?.at?.(-1)?.status === "passed" ? <button className="is-primary" onClick={() => action(approveCapabilityReady)}>批准为可引用能力</button> : null}</> : null}<button onClick={() => onContinue?.(selected)}>继续讨论</button></div></section></FounderWorkspaceInspector>;
}
