...
export function ModelCenter({ onContextChange, onHome }) {
  ...
  return <section className="sino-model-center sino-settings" aria-label="设置">
    ...
    <nav className="sino-settings-tabs" aria-label="设置分类">{[["models", "模型与 API"], ["model-capabilities", "模型能力"], ["model-routing", "模型路由策略"], ["sino", "Sino AI"], ["executor", "执行器"], ["discussion", "讨论配置"], ["runtime", "运行环境"]].map(([key, label]) => <button type="button" key={key} className={section === key ? "is-active" : ""} onClick={() => { setSection(key); setEditing(null); setSelectedModel(null); }}>{label}</button>)}</nav>
    ...
    {section === "model-capabilities" ? <section className="sino-settings-page" aria-label="模型能力页面"><ModelCapabilities registry={center.model_capability_registry} /></section> : null}
    {section === "model-routing" ? <section className="sino-settings-page" aria-label="模型路由策略页面"><RoutingPolicies registry={center.model_capability_registry} models={center.model_capability_registry?.models || []} busy={busy} onSave={savePreferred} /></section> : null}
...
function ModelCapabilities({ registry }) {
  ...
  return <section className="sino-model-capabilities sino-model-section-panel" aria-label="模型能力"><p className="sino-model-section-description">只读显示已有真实探测或验证证据的模型能力。</p><div>{registry.models.filter((model) => model.selected).map((model) => { const verifiedFields = fields.filter(([field]) => model.capabilities[field]?.source !== "INFERRED_UNVERIFIED"); return <article key={`${model.provider_id}:${model.model_id}`}><header><strong>{model.display_name}</strong><span>{model.healthy ? "正常" : "不可用"}</span></header><div>{verifiedFields.length ? verifiedFields.map(([field, label]) => <span key={field} data-status={model.capabilities[field].status}><b>{label}</b>{capabilityStatus(model.capabilities[field].status)}</span>) : <span>暂无已验证能力</span>}</div></article>; })}</div></section>;
}
