...
function RuntimeEnvironmentSettings({ registry }) {
  if (!registry) return <section className="sino-capability-section" aria-label="运行环境"><h3>运行环境</h3><p>正在读取运行环境注册表…</p></section>;
  ...
  return <section className="sino-capability-section" aria-label="运行环境"><div className="sino-capability-section-heading"><div><h3>运行环境</h3><p>统一管理 Sino Founder AI 当前依赖的运行环境事实。</p></div></div><div className="sino-runtime-stage-grid"><article><small>当前环境</small><strong>LOCAL</strong><span>{local.status}</span></article><article><small>下一阶段规划</small><strong>NAS</strong><span>{nas.status}</span></article><article><small>商业云</small><strong>未配置</strong><span>{cloud.status}</span></article></div><div className="sino-runtime-binding-list">{[["前端", `${frontend.protocol}://${frontend.host}:${frontend.port}`, frontend], ["后端", `${backend.protocol}://${backend.host}:${backend.port}`, backend], ["数据库", `${local.database?.type} / LOCAL`, local.database], ["身份与访问管理", local.iam?.type, local.iam], ["网络", local.network?.boundary, local.network]].map(([label, binding, item]) => <article key={label}><div><strong>{label}</strong><span>{binding}</span></div><dl><div><dt>状态</dt><dd>{item?.status || item?.verification_status || item?.connectivity_status}</dd></div><div><dt>健康状态</dt><dd>{item?.health || item?.health_status || "verified"}</dd></div><div><dt>最近验证</dt><dd>{item?.last_verified_at || "—"}</dd></div>{label === "数据库" ? <div><dt>凭据</dt><dd>{item?.credential_reference_exists ? "已配置 / 引用存在" : "未配置"}</dd></div> : null}</dl></article>)}</div></section>;
}
...
function DiscussionSettingsContext({ detail }) {
  ...
  return <><span className="sino-kicker">讨论配置</span><h3>讨论配置</h3><p className="sino-settings-provider-name">多模型讨论与自动多轮共用参与模型。</p><MultiModelSelect role={council} options={detail.options} onSave={detail.onSave} /></>;
}
