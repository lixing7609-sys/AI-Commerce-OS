...
function AssignmentStatus({ view, label, kind }) {
  return <span className={`sino-model-status sino-model-${kind}-status`} data-status={view.key} aria-label={`${label} ${kind === "primary" ? "Primary" : kind === "fallback" ? "Fallback" : "Assignment"} 状态`}><span className="sino-assignment-status-dot" aria-hidden="true">●</span>{" "}<span className="sino-assignment-status-label">{view.label}</span></span>;
}
...
const stableAssignmentRoleOrder = ["Sino 主对话", "深度推理", "Vision", "Coding", "讨论模型 1", "讨论模型 2", "讨论模型 3", "讨论模型 4", "讨论模型 5"];
export function formatStableAssignmentRoles(roles = []) {
  const stable = new Set(roles.map((role) => role.replace(/ Fallback$/, "")).filter((role) => stableAssignmentRoleOrder.includes(role)));
  return stableAssignmentRoleOrder.filter((role) => stable.has(role));
}
...
export function ModelCenter({ onHome }) {
  ...
  return <section className="sino-model-center sino-settings" aria-label="设置">
    ...
    <section className="sino-capability-section sino-settings-page sino-settings-page--models" aria-label="模型">
      ...
      <ModelEconomics economics={center.model_economics || { models: [], orphan_references: [] }} getAssignmentRoles={modelDuties} />
    </section>
...
function ModelEconomics({ economics, getAssignmentRoles }) {
...
function RuntimeEnvironmentSettings({ registry }) {
  ...
  return <section className="sino-capability-section sino-settings-domain-panel" aria-label="运行环境"><div className="sino-capability-section-heading"><div><h3>Runtime</h3><p>当前真实注册环境与系统健康。</p></div></div><div className="sino-runtime-summary"><strong>LOCAL</strong><span>{local.status}</span><span>{healthy}/{services.length} services healthy</span></div><div className="sino-runtime-binding-list">{[["前端", `${frontend.protocol}://${frontend.host}:${frontend.port}`, frontend], ["后端", `${backend.protocol}://${backend.host}:${backend.port}`, backend], ["数据库", `${local.database?.type} / LOCAL`, local.database], ["身份与访问管理", local.iam?.type, local.iam], ["网络", local.network?.boundary, local.network]].map(([label, binding, item]) => <article className="sino-settings-section-card" key={label}><div className="sino-runtime-service-header"><strong>{label}</strong><span>{binding}</span></div><dl className="sino-runtime-service-meta"><div><dt>状态</dt><dd>{item?.status || item?.verification_status || item?.connectivity_status}</dd></div><div><dt>健康</dt><dd>{item?.health || item?.health_status || "verified"}</dd></div><div><dt>最近验证</dt><dd>{formatRuntimeTimestamp(item?.last_verified_at)}</dd></div></dl></article>)}</div></section>;
}
...
function ModelChoices({ models, provider, onChoose, busy, catalogGrid = false }) {
  if (catalogGrid) return <div className="sino-model-choices sino-provider-model-card-grid">{models.map((model) => { const selected = provider.selected_models.includes(modelId(model)); return <article className={selected ? "is-selected" : ""} key={modelId(model)}><input type="checkbox" aria-label={`${model.display_name || modelId(model)} ${modelId(model)}`} checked={selected} onChange={(event) => onChoose(provider, model, event.target.checked)} disabled={Boolean(busy)} /><span><strong>{model.display_name || modelId(model)}</strong><em>{modelId(model)}</em></span>{model.recommendation_score >= 80 && <b>推荐</b>}</article>; })}</div>;
