...
export function SettingsContext({ detail, onClose, dialogRef }) {
  if (detail?.section !== "models" || !detail?.provider || !detail?.model) return null;
  return <div className="sino-add-model-overlay" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose?.(); }}><section ref={dialogRef} className="sino-add-model-modal sino-provider-config-modal sino-settings-context" role="dialog" aria-modal="true" aria-labelledby="provider-config-title" tabIndex={-1}><header className="sino-settings-context-header"><div><h2 id="provider-config-title">Provider 技术配置</h2><p>连接、模型启用与健康检查。</p></div><button type="button" onClick={onClose} aria-label="关闭 Provider 技术配置">×</button></header><article><ProviderSettingsContext detail={detail} /></article></section></div>;
}
