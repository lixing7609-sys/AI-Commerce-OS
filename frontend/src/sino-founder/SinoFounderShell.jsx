...
export function SinoFounderShell({ active, onNavigate, sidebarProps, main, context }) {
  ...
  return <div ref={shellRef} className={`sino-founder-shell${dragging ? " is-resizing" : ""}`} style={{ "--sidebar-width": `${sidebarWidth}px`, "--context-width": `${contextWidth}px` }}>
    ...
    <main ref={mainRef} className={`sino-founder-main${["conversation", "project", "object", "capability-center", "execution", "assets", "builder"].includes(active) ? " sino-founder-main--fixed-workspace" : ""}`} tabIndex={0} aria-label="Founder AI 工作区内容">{main}</main>
    ...
    <aside className={`sino-founder-context${active === "objects" ? " sino-founder-context--object-inspector" : ""}`} aria-label="当前上下文">{context}</aside>
  </div>;
}
