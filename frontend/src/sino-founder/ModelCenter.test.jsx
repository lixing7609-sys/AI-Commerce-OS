...
const connected = [deepseek, claude].flatMap((provider) => provider.selected_models.map((model) => ({ identity: `${provider.provider_key}::${model}`, provider_id: provider.provider_key, model_id: model, display_name: provider.available_models.find((item) => item.model_id === model)?.display_name || model, provider_name: provider.display_name, health_status: provider.health_status, availability: provider.health_status === "healthy" ? "available" : "unavailable" })));
const eligibleVision = [{ identity: "ofox::gemini-3.6-flash", provider_id: "ofox", model_id: "gemini-3.6-flash", display_name: "Gemini 3.6 Flash", provider_name: "Ofox", health_status: "healthy", availability: "available" }, { identity: "ofox::google/gemini-3.1-flash-image", provider_id: "ofox", model_id: "google/gemini-3.1-flash-image", display_name: "Google/gemini 3.1 Flash Image", provider_name: "Ofox", health_status: "healthy", availability: "available" }];
const eligibleModels = { sino_conversation: connected, deep_thinking: connected, code_execution: connected, multi_model_discussion: connected, vision: eligibleVision };
const economics = { telemetry_status: "enabled", connected_model_count: 1, valid_assigned_model_count: 1, orphan_reference_count: 0, orphan_references: [], models: [{ ...connected[0], roles: ["Sino 主对话", "Coding", "讨论模型 1"], request_count: 0, completed_request_count: 0, total_tokens: null, average_latency_ms: null, cost: null, telemetry_status: "enabled_no_records", token_status: "enabled_no_records", pricing_status: "not_configured" }] };
const center = { provider_catalog: [{ provider_type: "openai", display_name: "OpenAI", default_base_url: "https://api.openai.com/v1", requires_base_url: false }, { provider_type: "anthropic", display_name: "Anthropic", default_base_url: "https://api.anthropic.com/v1", requires_base_url: false }, { provider_type: "ofoxai", display_name: "OfoxAI", default_base_url: "", requires_base_url: true }], providers: [deepseek], connected_models: connected, eligible_models: eligibleModels, model_economics: economics, roles: capabilities, agents, skills, applications: [{ application_key: "founder_ai", label: "Founder AI", assignments: appAssignments }, { application_key: "operator_ai", label: "Operator AI", assignments: appAssignments }], health_cost: [{ ...deepseek, usage: { calls: 3, tokens: null, cost: null, average_latency_ms: 120.5, quota: null } }], execution_engines: [{ engine_id: "codex", display_name: "Codex", status: "available", engine_type: "codex" }] };
...
describe("Founder Settings", () => {
  ...
  it("keeps unknown tokens and unknown cost distinct from zero", async () => {
    getModelCenter.mockResolvedValue({
      ...center,
      model_economics: {
        ...economics,
        models: [
          { ...connected[0], roles: ["Sino 主对话"], request_count: 0, completed_request_count: 0, total_tokens: null, average_latency_ms: null, cost: null, telemetry_status: "enabled_no_records", token_status: "enabled_no_records", pricing_status: "configured_no_usage" },
          { provider_id: "claude", model_id: "claude-sonnet-5", display_name: "Claude Sonnet 5", provider_name: "Claude", roles: ["Coding"], request_count: 1, completed_request_count: 0, total_tokens: null, average_latency_ms: null, cost: null, health_status: "unhealthy", telemetry_status: "recorded", token_status: "unavailable", pricing_status: "token_unavailable" },
          { provider_id: "gpt", model_id: "gpt-5", display_name: "GPT 5", provider_name: "GPT", roles: ["讨论模型 1"], request_count: 1, completed_request_count: 1, total_tokens: 42, average_latency_ms: 88, cost: null, health_status: "healthy", telemetry_status: "recorded", token_status: "recorded", pricing_status: "not_configured" },
        ],
      },
    });
    render(<ModelCenter />);
    ...
    expect(table.textContent).toContain("无调用记录");
    expect(table.textContent).toContain("无调用成本");
  ...
  it("shows assigned-model economics without reusing incomplete Provider-level metrics", async () => {
    ...
    render(<ModelCenter />); await screen.findByRole("heading", { name: "设置" });
    expect(screen.getByRole("list", { name: "已接入模型列表" })).toBeTruthy();
    expect(screen.getByText("Invocation 覆盖", { exact: true })).toBeTruthy();
    expect(screen.getByText("Token 覆盖", { exact: true })).toBeTruthy();
    expect(screen.getByText("Pricing 覆盖", { exact: true })).toBeTruthy();
