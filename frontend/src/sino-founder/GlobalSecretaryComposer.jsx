...
export function GlobalSecretaryComposer({ value, onChange, onSubmit, busy, large = false, healthy, toolbar = null, toolbarIncludesStatus = false, placeholder = "和 Sino 讨论任何想法、问题、战略或设计……", mode = "sino", onModeChange, disabledModes = [], attachments = [], onAddImages, onRemoveImage }) {
  ...
  const attachmentUi = onAddImages ? <><input ref={fileRef} className="sino-image-file-input" type="file" accept="image/png,image/jpeg,image/webp" multiple onChange={(event) => { acceptImages(event.target.files || []); setAttachmentError(""); event.target.value = ""; }} /><div className="sino-composer-attachments">{attachments.map((item, index) => <figure key={item.localId || item.name || index}><img src={item.preview} alt={item.name || "待发送截图"} /><figcaption>{item.name || item.file?.type || "截图"}</figcaption><button type="button" aria-label={`删除图片 ${index + 1}`} onClick={() => onRemoveImage?.(index)}>×</button></figure>)}</div>{attachmentError ? <p className="sino-composer-attachment-error" role="alert">{attachmentError}</p> : null}</> : null;
  ...
  function handlePaste(event) {
    ...
    const imageItems = items.filter((item) => ["image/png", "image/jpeg", "image/webp"].includes(item.type));
    if (!imageItems.length) return;
    event.preventDefault();
    const files = imageItems.map((item) => item.getAsFile?.()).filter(Boolean);
    if (!files.length || !onAddImages) { setAttachmentError("图片粘贴失败，请重试或使用图片按钮选择。"); return; }
    const pastedText = event.clipboardData?.getData?.("text/plain") || "";
    if (pastedText) {
      const input = event.currentTarget; const start = input.selectionStart ?? value.length; const end = input.selectionEnd ?? start;
      onChange(`${value.slice(0, start)}${pastedText}${value.slice(end)}`);
    }
    acceptImages(files); setAttachmentError("");
  }
  useLayoutEffect(() => {
  ...
  if (!toolbar) return <form className={`sino-composer sino-global-composer${large ? " sino-global-composer--large" : ""}`} onSubmit={onSubmit} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); acceptImages(event.dataTransfer.files); }}>
    {modeControl || <label htmlFor={inputId}>{status}</label>}
    {attachmentUi}
    <div>
      <textarea ref={textareaRef} id={inputId} aria-label="讨论内容" value={value} onChange={(event) => onChange(event.target.value)} onPaste={handlePaste} onKeyDown={handleKeyDown} placeholder={placeholder} rows={large ? 5 : 2} />
  ...
  return <form className={`sino-composer sino-global-composer sino-global-composer--toolbar${large ? " sino-global-composer--large" : ""}`} onSubmit={onSubmit} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); acceptImages(event.dataTransfer.files); }}>
