...
import { createFounderConversation, createFounderExecution, createFounderProject, discussWithCouncil, discussWithSino, getAssetMemoryCenter, getConversationWorkspace, getFounderBriefing, getFounderExecution, getFounderProjects, getFounderStrategy, getLibraryArtifact, getModelCenter, getProjectIntelligence, reasonConfirmedGoal, retryCouncil, retrySinoReply, submitExecutionDelta } from "../services/founderAiApi.js";
import { createTaskAsset } from "../services/taskAssetApi.js";

vi.mock("../services/founderAiApi.js", () => ({ approveFounderExecution: vi.fn(), buildSystemBlueprint: vi.fn(), confirmCandidateGoal: vi.fn(), createFounderConversation: vi.fn(), createFounderExecution: vi.fn(), createFounderProject: vi.fn(), decideExecutionDelta: vi.fn(), discussWithAutoDeliberation: vi.fn(), discussWithCouncil: vi.fn(), discussWithSino: vi.fn(), getAssetMemoryCenter: vi.fn(), getConversationWorkspace: vi.fn(), getFounderBriefing: vi.fn(), getFounderExecution: vi.fn(), getFounderProjects: vi.fn(), getFounderStrategy: vi.fn(), getLibraryArtifact: vi.fn(), getLibraryMemory: vi.fn(), getModelCenter: vi.fn(), getProjectIntelligence: vi.fn(), createArtifactVersion: vi.fn(), createIntelligenceReference: vi.fn(), createMemoryRevision: vi.fn(), mergeLibraryMemories: vi.fn(), updateArtifactStatus: vi.fn(), updateMemoryStatus: vi.fn(), reasonConfirmedGoal: vi.fn(), resumeFounderExecution: vi.fn(), retryCouncil: vi.fn(), retrySinoReply: vi.fn(), submitExecutionDelta: vi.fn() }));
...
describe("Sino Founder AI interaction responsibilities", () => {
  it("maps legacy blueprint and capability views to their top-level owners", () => {
    expect(normalizeFounderView("blueprint")).toBe("builder");
    expect(normalizeFounderView("system-builder")).toBe("builder");
    expect(normalizeFounderView("capability")).toBe("capability-center");
    expect(normalizeFounderView("models")).toBe("capability-center");
  });
  ...
  it("keeps one Global Shell while primary views change their context panel", async () => {
    ...
    expect(within(screen.getByRole("navigation", { name: "Founder AI 能力入口" })).queryByRole("button", { name: "战略与路线" })).toBeNull();
