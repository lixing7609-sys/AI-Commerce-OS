...
function OperationalRuntimeStatus({ runtime }) {
  ...
  const labels = {
    ...
    approval_required: "等待 Founder 批准",
...
function MissionStatus({ mission }) {
  ...
  const refreshedMerge = mission.refreshed_safe_merge_validation;
  return <aside className="sino-operational-runtime-status" aria-label="Mission Status">
    ...
    {refreshedMerge ? <section aria-label="Refreshed Merge Validation"><h4>Integration baseline 已更新</h4><p>Sino 已基于最新 baseline 重新验证该合并。</p><dl>{refreshedMerge.status ? <div><dt>Refreshed validation</dt><dd>{refreshedMerge.status}</dd></div> : null}{refreshedMerge.source_head ? <div><dt>Source HEAD</dt><dd>{refreshedMerge.source_head}</dd></div> : null}{refreshedMerge.target_head ? <div><dt>Target HEAD</dt><dd>{refreshedMerge.target_head}</dd></div> : null}{refreshedMerge.merge_base ? <div><dt>Merge base</dt><dd>{refreshedMerge.merge_base}</dd></div> : null}</dl></section> : null}
...
export function ArchitectureProposalCard({ proposal, busy, onDecision }) {
...
export function ConversationThread({ snapshot, drafts = [], onOpenDraft, message, onMessage, onSend, busy, replyPending = false, streamingReply = null, mode, onModeChange, healthy, contextControls, onExitObjectDiscussion, onConfirmGoal, onReviseGoal, onAdvanceStage, onReviewPackage, onReviewConstitution, onReviewProjectOutcome, onReviewImplementationPlan, onContinueProjectAnalysis, onReviewFounderGate, onImageProbeDecision, onExternalProbeDecision, onArchitectureDecision, selectedConstitutionWorkItemId, onSelectConstitutionWorkItem, onContinueDiscussion, onViewAssets, onNewGoal, capabilityAction, capabilityAsset, capabilityError, onCapabilityAction, reuseSuggestions, onReuse, pendingAttachments = [], onAddImages, onRemoveImage }) {
  ...
  const hasStageProjection = false;
  ...
  const autonomousMissionId = autonomousMission?.mission_id;
  const autonomousMissionSourceMessageId = autonomousMission?.source_message_id;
  const missionAnchorMessageId = autonomousMission?.acknowledgement_message_id
    || visibleMessages.find((item) => item.role === "assistant" && item.grounding?.mission?.mission_id === autonomousMissionId)?.message_id
    || visibleMessages.find((item) => item.role === "assistant" && item.grounding?.mission?.source_message_id === autonomousMissionSourceMessageId)?.message_id
    || null;
  const missionHasAnchor = Boolean(autonomousMission?.mission_id && missionAnchorMessageId);
  const missionStatusForMessage = (item) => item.message_id === missionAnchorMessageId ? <MissionStatus mission={autonomousMission} /> : null;
