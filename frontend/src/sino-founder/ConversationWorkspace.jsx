...
import { approveFounderObject, archiveFounderObject, bindFounderConversationProject, clearFounderObjectDiscussion, continueFounderCandidateDiscussion, continueFounderObjectDiscussion, createTaskFromFounderObject, getFounderObject, reviewFounderCandidate } from "../services/founderAiApi.js";
...
import { AnchoredComposerContextControls, ProjectWorkspace } from "./FounderHome.jsx";
...
import { ExecutionCenterEmpty, SinoBrainContext } from "./SinoBrainContext.jsx";
...
export const normalizeFounderView = (next) => {
  if (["home", "draft", "project"].includes(next)) return "conversation";
  ...
  if (next === "lifecycle") return "conversation";
  return next;
};
...
export function ConversationWorkspace() {
  ...
  const [projectLanding, setProjectLanding] = useState(false);
  ...
  const recoverMissingProject = useCallback(() => {
    ...
    forgetProject();
    persistWorkspace("conversation", null);
  ...
  async function sendDiscussion(event) {
    ...
    try {
      ...
      if (isFirstSubmit && !id) {
        pendingClientMessageIdRef.current = null; setSnapshot(null); setDiscussionMessage(content); setReplyPending(false); setPendingClientMessageId(null); setStreamingReply(null); setView("conversation");
      }
      if (id) {
        try {
  ...
  function newConversation(preserveProject = false) {
    ...
    remember(CONVERSATION_KEY, null); remember(EXECUTION_KEY, null); remember(WORKSPACE_VIEW_KEY, "conversation"); setCreationContext(null); setSelectedWorkspaceObject(null); setProjectLanding(false); setView("conversation");
  }
  ...
  async function confirmDeleteConversation() {
    ...
    try {
      ...
      if (id === conversationId) {
        ...
        if (fallback) await selectConversation(fallback.id); else { setActiveProjectId(null); setProjectIntelligence(null); setView("conversation"); }
      }
  ...
  async function createTaskFromObject(item) {
    setBusy(true); setError("");
    try {
      const bridge = await createTaskFromFounderObject(item.object_id);
      const restored = conversationId ? await getConversationWorkspace(conversationId) : null;
      setSnapshot(restored || ((current) => ({ ...current, founder_objects: (current?.founder_objects || []).map((value) => value.object_id === bridge.object_id ? { ...value, task_asset_ref: { task_id: bridge.task_id, title: bridge.title, status: bridge.status, approval_status: bridge.approval_status, execution_status: bridge.execution_status, created_from: "founder_object_bridge" } } : value) })));
    }
    catch (requestError) { setError(requestError.message); }
    finally { setBusy(false); }
  }
  ...
  function openProject(id) {
    ...
    remember(CONVERSATION_KEY, null); remember(EXECUTION_KEY, null); remember(PROJECT_KEY, id); setProjectLanding(true); setView("conversation");
  }
  ...
  const currentConversationCandidates = (snapshot?.object_candidates || []).filter((item) => !conversationId || !item.conversation_id || item.conversation_id === conversationId);
  const currentContextCandidate = snapshot?.context_candidate && (!conversationId || !snapshot.context_candidate.conversation_id || snapshot.context_candidate.conversation_id === conversationId) ? snapshot.context_candidate : null;
  const hasCandidateReview = currentConversationCandidates.some((item) => item.review_status === "pending") || currentContextCandidate?.review_status === "pending";
  const reloadFounderWorkspace = async () => {
    if (!conversationId) return;
    setSnapshot(await getConversationWorkspace(conversationId));
  };
  const candidateReviewContext = hasCandidateReview ? <ImplementationWorkspace objects={snapshot?.founder_objects || []} candidates={currentConversationCandidates} conversationId={conversationId} contextObject={snapshot?.context_object || null} contextCandidate={currentContextCandidate} intelligence={conversationIntelligence} creationContext={creationContext} recognitionStatus={snapshot?.object_recognition || null} onApprove={approveObject} onCreateTask={createTaskFromObject} onContinue={continueObject} onArchive={archiveObject} onOpenObject={openCapabilityObject} onCandidateReview={reviewCandidate} onCandidateContinue={continueCandidate} onTaskLifecycleChanged={reloadFounderWorkspace} busy={busy} /> : null;
  const capabilityContext = <>{candidateReviewContext}{snapshot?.sino_brain ? <SinoBrainContext brain={snapshot.sino_brain} conversationId={conversationId} contextGroundings={contextGroundings} busy={busy} capabilityAction={capabilityAction} capabilityAsset={activeCapabilityAsset} selectedConstitutionWorkItemId={selectedConstitutionWorkItemId} onReviewConstitutionWorkItem={reviewConstitutionWorkItemDecision} onReviewConstitutionRouting={reviewConstitutionRoutingDecision} onConfirmFormalObject={confirmConstitutionFormalObject} onOpenProject={openProject} onCapabilityAction={handleCapabilityAction} onConfirmGoal={confirmBrainGoal} onForceReview={forceBrainGoalReview} onStartStrategy={startBrainStrategy} onAdvanceStage={advanceBrainStage} onContinueDiscussion={continueBrainDiscussion} onReviewPackage={reviewBrainPackage} onViewAssets={viewTaskResult} onNewGoal={newConversation} onExternalProbeDecision={decideCurrentExternalProbeGate} onImageProbeDecision={decideCurrentImageProbeGate} onArchitectureDecision={decideCurrentArchitectureProposal} onTaskAccepted={setSnapshot} onClarificationResolved={reloadFounderWorkspace} onTaskCandidateResolved={reloadFounderWorkspace} onFounderActionResolved={reloadFounderWorkspace} /> : <ExecutionCenterEmpty />}</>;
  const contextControls = <AnchoredComposerContextControls healthy={sinoHealthy} projects={projects} activeProjectId={snapshot?.conversation?.project_id || activeProjectId || null} onSelectProject={conversationId ? bindCurrentConversationProject : selectProjectContext} onCreateProject={createProject} onFiles={addPendingImages} onSelectDocument={openConversationFiles} />;
  let main = selectedFounderGateProposal ? <FounderGateProposalReview proposal={selectedFounderGateProposal} busy={busy} onReview={reviewCurrentFounderGateProposal} onReturnDiscussion={() => setSelectedFounderGateProposal(null)} /> : <section className="sino-conversation-page"><ConversationThread snapshot={snapshot} drafts={drafts} onOpenDraft={(draft) => { setSelectedDraft(draft); setRepositorySection("drafts"); setView("capability-center"); }} message={discussionMessage} onMessage={setDiscussionMessage} onSend={sendDiscussion} busy={busy} replyPending={replyPending} streamingReply={streamingReply} capabilityAction={capabilityAction} capabilityAsset={activeCapabilityAsset} capabilityError={capabilityLifecycleError} onCapabilityAction={handleCapabilityAction} reuseSuggestions={reuseSuggestions} onReuse={handleReuseAsset} healthy={sinoHealthy} contextControls={contextControls} mode={discussionMode} onModeChange={setDiscussionMode} onExitObjectDiscussion={exitObjectDiscussion} onConfirmGoal={confirmBrainGoal} onReviseGoal={() => setDiscussionMessage("这里需要修正：")} onAdvanceStage={advanceBrainStage} onReviewPackage={reviewBrainPackage} onReviewConstitution={reviewConstitution} onReviewProjectOutcome={reviewPlanningOutcome} onReviewImplementationPlan={reviewCurrentImplementationPlan} onContinueProjectAnalysis={continueProjectPlanning} onReviewFounderGate={openFounderGateProposal} onImageProbeDecision={decideCurrentImageProbeGate} onExternalProbeDecision={decideCurrentExternalProbeGate} onArchitectureDecision={decideCurrentArchitectureProposal} selectedConstitutionWorkItemId={selectedConstitutionWorkItemId} onSelectConstitutionWorkItem={selectConstitutionWorkItem} onContinueDiscussion={continueBrainDiscussion} onViewAssets={viewTaskResult} onNewGoal={newConversation} pendingAttachments={pendingAttachments} onAddImages={addPendingImages} onRemoveImage={removePendingImage} /></section>;
  let context = capabilityContext;
  if (view === "conversation" && projectLanding && !conversationId) { main = <ProjectWorkspace intelligence={projectIntelligence ? { ...projectIntelligence, conversation_refs: conversations.filter((item) => item.project_id === activeProjectId).map((item) => ({ conversation_id: item.id, title: item.title, summary: item.summary || item.preview || item.last_message_preview, updated_at: item.updatedAt || item.updated_at })) } : null} loading={projectLoading} error={projectLoadError} onOpenConversation={selectConversation} message={discussionMessage} onMessage={setDiscussionMessage} onSend={sendDiscussion} busy={busy} healthy={sinoHealthy} mode={discussionMode} onModeChange={setDiscussionMode} />; }
