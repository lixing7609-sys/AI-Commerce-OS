...
function ObjectCard({ item, taskRef, selected, onSelect }) {
  return <button type="button" className={`sino-implementation-card${selected ? " is-active" : ""}`} onClick={() => onSelect(item.object_id)}>
    ...
    {taskRef ? <em>TaskAsset：{statusLabel(taskRef.status)} · 审批 {statusLabel(taskRef.approval_status)} · 执行 {taskExecutionLabel(taskRef.execution_status)}</em> : null}
...
export function ImplementationWorkspace({ objects = [], candidates = [], conversationId = null, contextObject = null, contextCandidate = null, intelligence = null, creationContext = null, recognitionStatus = null, onApprove, onCreateTask, onContinue, onArchive, onOpenObject, onCandidateReview, onCandidateContinue, onApproveTaskExecution = approveTaskForExecution, onRejectTaskExecution = rejectTaskForExecution, onStartTaskExecution = startTaskExecution, onTaskLifecycleChanged, busy }) {
  ...
  const effectiveTaskRef = (item) => {
    const ref = taskAssetRef(item);
    const id = taskAssetId(ref);
    return id && taskApprovalUpdates[id] ? { ...ref, ...taskApprovalUpdates[id] } : ref;
  };
  const selectedTaskRef = effectiveTaskRef(selected);
  const reviewTaskExecution = async (decision) => {
  ...
  const effectiveBusy = busy || taskApprovalBusy;
  return <section className="sino-object-workspace sino-capability-context" aria-label="能力上下文">
    ...
    <div className="sino-object-workspace__list">
      ...
      {group("已批准对象", approved)}
      {contextObject && !pendingTargetIds.has(contextObject.object_id) ? <section className="sino-object-workspace__group"><h3>当前对象</h3><ObjectCard item={contextObject} taskRef={effectiveTaskRef(contextObject)} selected={selected?.object_id === contextObject.object_id} onSelect={setSelectedId} /></section> : null}
