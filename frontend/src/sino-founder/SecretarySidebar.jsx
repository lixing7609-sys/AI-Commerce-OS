...
export function SecretarySidebar({ onNavigate, conversations = [], activeConversationId, onNewConversation, onSelectConversation, onDeleteConversation, projects = [], activeProjectId, onSelectProject, onProjectsChanged }) {
  ...
  const unassignedConversations = founderConversations.filter((item) => !projectForConversation(item));
  ...
  return <aside className={`sino-sidebar${collapsed ? " sino-sidebar--collapsed" : ""}`}>
    ...
    <div className="sino-sidebar__scroll-region" aria-label="历史会话列表">
      <div className="sino-conversation-navigation">
        <ConversationList items={unassignedConversations} now={now} projects={sortedProjects} activeConversationId={activeConversationId} onSelectConversation={onSelectConversation} onDeleteConversation={onDeleteConversation} onMoveConversation={moveConversation} />
      </div>
    </div>
