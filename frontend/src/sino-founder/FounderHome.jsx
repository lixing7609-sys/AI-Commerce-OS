...
export function ProjectWorkspace({ intelligence, drafts = [], loading, error, onOpenConversation, onOpenDraft, message, onMessage, onSend, busy, healthy, mode, onModeChange }) {
  ...
  return <section className="sino-project-workspace" aria-label="项目工作区">
    ...
    {intelligence.project_lifecycle?.rank >= 300 ? <section className="sino-project-drafts" aria-label="项目生命周期"><header><strong>{intelligence.project_lifecycle.stage_label}</strong><span>{intelligence.project_lifecycle.current_action?.status_label}</span></header><p>{intelligence.project_lifecycle.current_action?.description}</p></section> : null}
