...
test("Settings exposes model control and system health with the real Provider inspector", async ({ page }) => {
  ...
  await expect(providerDialog.locator(".sino-provider-summary-row")).toHaveCount(3);
  ...
  await expect(statusRow.getByText("Provider", { exact: true })).toBeVisible();
  ...
  const endpointRow = providerDialog.getByText("Base URL", { exact: true }).locator("xpath=parent::div");
  await expect(endpointRow).toContainText("https://api.deepseek.com/v1");
