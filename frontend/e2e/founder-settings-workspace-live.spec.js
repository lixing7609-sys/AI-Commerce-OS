...
test("Settings exposes model control and system health with the real Provider inspector", async ({ page }) => {
  ...
  const providerModelGrid = providerDialog.locator(".sino-provider-model-card-grid");
  ...
  const catalogCheckbox = providerModelGrid.getByRole("checkbox", { name: "DeepSeek Model 3 deepseek-model-3" });
  ...
  const apiKeyRow = providerDialog.locator(".sino-provider-summary-row--actions");
  await expect(apiKeyRow.getByText("****1234")).toBeVisible();
  await expect(apiKeyRow.getByRole("button", { name: "更新 API Key" })).toBeVisible();
  await expect(apiKeyRow.getByRole("button", { name: "测试连接" })).toBeVisible();
  ...
  await providerDialog.getByRole("button", { name: "查看全部 18 个模型" }).click();
  await expect(providerModelGrid.locator(":scope > article")).toHaveCount(18);
  expect((await providerModelGrid.evaluate((grid) => getComputedStyle(grid).gridTemplateColumns.split(" ").length))).toBe(4);
  await providerDialog.getByRole("button", { name: "收起" }).click();
