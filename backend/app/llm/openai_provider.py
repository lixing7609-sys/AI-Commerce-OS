...
class OpenAIProvider(LLMProvider):
    """
    OpenAI Chat Completions Provider（Sino GPT Brain 专用）。
    ...
    def generate(self, request: LLMRequest) -> LLMResponse:
        ...
        try:
            ...
            logger.error("openai unreachable: error_type=%s", type(error).__name__)
            raise ProviderUnavailableError() from error
        except httpx.HTTPError as error:
            logger.error("openai network error: error_type=%s", type(error).__name__)
            raise NetworkError() from error

        latency_ms = (time.monotonic() - started_at) * 1000

        logger.info(
            "openai response received: status=%s latency_ms=%.1f",
