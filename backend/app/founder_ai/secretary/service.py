...
class SinoSecretaryService:
    ...
    def snapshot(self, conversation_id: str) -> dict:
        with SessionLocal() as session:
            ...
            payload = {"conversation": {"id": conversation.id, "project_id": conversation.project_id, "title": conversation.title, "state": conversation.conversation_state, "conversation_model_provider": conversation.conversation_model_provider, "conversation_model": conversation.conversation_model, "updated_at": _iso(conversation.updated_at)}, "messages": [{"message_id": m.id, "role": m.role, "content": m.content, "message_type": m.message_type, "intent": m.intent, "grounding": dict(m.grounding or {}), "attachment_refs": attachment_map.get(m.id, []), "created_at": _iso(m.created_at)} for m in messages], "digest": digest_payload, "conversation_intelligence": conversation_intelligence, "project_context": project_context, "goals": goal_payload}
        try:
