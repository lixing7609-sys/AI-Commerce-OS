...
def execute_reused_runtime_validation(*, repo_root: Path) -> dict:
    ...
    path = repo_root / ".founder-execution" / "reuse-validations" / f"{record['task']['task_id']}.json"; path.parent.mkdir(parents=True, exist_ok=True); path.write_text(json.dumps(record, ensure_ascii=False, indent=2), encoding="utf-8")
    return record
