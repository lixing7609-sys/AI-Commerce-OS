...
from app.founder_ai.visual_grounding import GROUNDING_CONFIDENCE_THRESHOLD
...
def route_task_complexity(text: str, *, image_understanding: str | dict | None = None, image_context_status: str = "not_present") -> dict:
    ...
    grounded_area = grounded.get("target_area") or grounded.get("visual_location")
    grounded_text_intent = grounded.get("text_intent") or {}
    grounded_action = grounded_text_intent.get("operation")
    grounded_ready = bool(
        grounded_target and grounded_area and grounded_action and grounded_text_intent.get("action_clear") is True
        and grounding_confidence >= GROUNDING_CONFIDENCE_THRESHOLD
        and not re.search(r"unknown|unclear|multiple|不明确|多个", str(grounded_target), re.I)
    )
    clarification_required = (bool(grounded.get("clarification_required")) and not grounded_ready) or bool(image_context_status == "unavailable" and not quick and len(text.strip()) <= 12)
    ...
    if classification == QUICK_FIX:
        ...
        target_area = grounded.get("target_area") or grounded.get("visual_location") or target_area
        result["quick_fix_contract"] = {"issue_type": "UI_CLEANUP" if grounded.get("text_intent", {}).get("operation") == "REMOVE_UI_ELEMENT" or re.search(r"去掉|删除|移除", text) else "UI_INTERACTION_BUG" if re.search(r"折叠|click|按钮", combined, re.I) else "BOUNDED_UI_BUG",
            "target_area": target_area, "visual_target": grounded_target, "operation": grounded_action, "grounding_confidence": grounded.get("grounding_confidence"), "observed_problem": text,
            "expected_behavior": "AI Commerce OS project node toggles collapse / expand" if re.search(r"AI Commerce OS.*折叠|折叠.*AI Commerce OS", combined, re.I) else grounded.get("expected_change") or image_text or text,
            "constraints": list(grounded.get("constraints") or []), "grounded_quick_fix_intent": grounded,
            "clarification_reason": None if not clarification_required else grounded.get("clarification_reason") or "target_context_insufficient", "allowed_files_or_paths": ["frontend/src/sino-founder/**"],
