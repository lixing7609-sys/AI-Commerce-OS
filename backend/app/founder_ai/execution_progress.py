...
def build_execution_progress(route: dict) -> dict | None:
    ...
    if session and session.current_stage in {"scope_verification_started", "scope_verification_finished"}:
        route_step = "scope_verification"
    elif session and session.current_stage in {"scope_correction_started", "scope_correction_finished"}:
        route_step = "correcting_scope"
    ...
    if exhausted:
        ...
        if blocker.get("type") == "standard_task_scope_mismatch":
            title, next_action = "任务范围错位", "本次错误改动已隔离；等待进一步处理"
        elif route.get("execution_status") == "failed":
