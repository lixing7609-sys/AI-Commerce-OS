...
def test_vision_routes_text_and_image_in_one_request(monkeypatch):
    ...
    class Session:
        def scalar(self, query): return model
        def __enter__(self): return self
        def __exit__(self, *args): pass
    ...
    monkeypatch.setattr(vision_routing.llm_gateway, "generate_for_model", lambda p, m, request: (seen.update({"request": request}) or SimpleNamespace(provider=p, model=m, content="sidebar arrow")))
    ...
    assert result["understanding"] == "sidebar arrow"
