...
MOCK_JOB=1; MOCK_OWNER=1; MOCK_HTTP=1
job_loaded() { [ "$MOCK_JOB" -eq 1 ]; }
job_running() { [ "$MOCK_JOB" -eq 1 ]; }
job_pid() { echo 100; }
listener_owner_matches_tree() { [ "$MOCK_OWNER" -eq 1 ]; }
...
MOCK_FRONTEND_READY=0; MOCK_RUNTIME_JSON="$fresh_json"
